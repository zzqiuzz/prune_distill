...
parser.add_argument('--teacher_model_path',type=str,help='path to load teacher model.')
parser.add_argument('--kd',action='store_true',help='Knowledge Distillation')
parser.add_argument('--T',default=10,type=float,help='Temperature used for knowledge distillation')
parser.add_argument('--alpha',default=0.5,type=float,help='used for knowledge distillation')
...
def main():
    ...
    teacher = torch.nn.DataParallel(teacher,device_ids=list(range(args.ngpu)))
    if args.kd:
        if os.path.isfile(args.teacher_model_path):
            checkpoint = torch.load(args.teacher_model_path)
            #pdb.set_trace()
            teacher.load_state_dict(checkpoint['state_dict'])
            print_log("Teacher model loaded.",log)
        else:
            print_log("Teacher model load failed.",log)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_acc, train_los = train(train_loader, net,teacher, criterion, optimizer, epoch, log, m)
...
def studentLossFn(teacher_pred, student_pred, y, T, alpha):
    ...
    Loss = alpha * (distillation loss with soft-target) + (1 - alpha) * (cross-entropy loss with true label)
    Return: loss
    :type y: object
    """
    if (alpha > 0):
...
def train(train_loader, model,teacher, criterion, optimizer, epoch, log, m):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.kd:
            with torch.no_grad():
                teacher_pred = teacher(input)

        # compute output
        output = model(input)
        if args.kd:
            loss = studentLossFn(teacher_pred, output, target, args.T, args.alpha)
        else:
...
class Mask:
    def __init__(self, model):
        ...
        self.norm_matrix = {}
        self.log = None
        self.all_layer_fixed_index = {}
        self.layer_wise_flag = {}
        self.pre_layer_pruned_index = {}
    ...
    def get_filter_codebook(self,index, weight_torch, compress_rate, length):
        ...
        if len(weight_torch.size()) == 4:
            ...
            if self.layer_wise_flag[index] is False:
                filter_index = norm2_np.argsort()[:filter_pruned_num]
            else:
                filter_sorted_index = list(norm2_np.argsort())
                for id in list(self.all_layer_fixed_index[index]):
                    if id in filter_sorted_index:
                        filter_sorted_index.remove(id)
                filter_index = filter_sorted_index[:filter_pruned_num]
            #pdb.set_trace()
            if self.log is not None :
                print_log(np.sort(filter_index),self.log)
    ...
    def get_filter_similar(self,index, weight_torch, compress_rate, distance_rate, length, dist_type="l2"):
        ...
        if len(weight_torch.size()) == 4:
            ...
            #comment these lines below for random initialization.
            ...
            '''save index'''
            #if  self.layer_wise_flag[index] == False:
            ##    pdb.set_trace()
            #    self.pre_layer_pruned_index[index] = similar_index_for_filter
            #    self.layer_wise_flag[index] = True

            #else:
            ##    pdb.set_trace()
            #    ans = np.intersect1d(np.array(similar_index_for_filter),np.array(self.pre_layer_pruned_index[index]))
            #    if len(ans) == similar_pruned_num:
            #        if self.log is not None:
            #            print_log("prune thes same filter index as the former epoch", self.log)

            #'''random initialization'''
            #if self.layer_wise_flag[index] == False:
            #    randn_ints = np.arange(len(filter_large_index))
            #    np.random.shuffle(randn_ints)
            #    pruned_index = randn_ints[: similar_pruned_num]
            #    similar_index_for_filter = [filter_large_index[i] for i in pruned_index]
            #    self.all_layer_fixed_index[index] = similar_index_for_filter
            #    self.layer_wise_flag[index] = True
            #else:
            #    similar_index_for_filter = self.all_layer_fixed_index[index]
            '''print'''
            ...
            if self.log is not None :
                print_log(np.sort(similar_index_for_filter),self.log)
            kernel_length = weight_torch.size()[1] * weight_torch.size()[2] * weight_torch.size()[3]
    ...
    def init_flag(self):
        for key in range(args.layer_begin, args.layer_end + 1, args.layer_inter):
            self.layer_wise_flag[key] = False
    ...
    def init_mask(self, rate_norm_per_layer, rate_dist_per_layer, dist_type):
        ...
        for index, item in enumerate(self.model.parameters()):
            if index in self.mask_index:
                ...
                self.mat[index] = self.get_filter_codebook(index, item.data, self.compress_rate[index],
                ...
                self.similar_matrix[index] = self.get_filter_similar(index,item.data, self.compress_rate[index],
